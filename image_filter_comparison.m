function image_filter_comparison(original_image, ...
    original_image_matrix, filtered_image_matrix, filter_type, ...
    window_size,std_dev)
%==========================================================================
...
%
% Arguments:
%   origional_image             original pgm image        
%   origional_image_matrix      original image as matrix
...
subplot(2,2,1), imshow(original_image), title("Original Image"); hold on

% if gausian filtering used, display standard deviation with filtered image
% results, otherise only show window size
if filter_type == "gaussian"
    subplot(2,2,2), imshow("filtered_image.pgm"), ...
        title( filter_type + " filtered, window size: " + ...
        window_size + " σ = " + std_dev); hold on
else
    subplot(2,2,2), imshow("filtered_image.pgm"), ...
        title( filter_type + " filtered, window size: " +...
        window_size); hold on
...
subplot(2,2,3), edge(original_image_matrix, "canny"), ...
    title("Original Image canny edge detection"); hold on
subplot(2,2,4), edge(filtered_image_matrix, "canny"), ...
    title("Filtered Image canny edge detection"); hold off
