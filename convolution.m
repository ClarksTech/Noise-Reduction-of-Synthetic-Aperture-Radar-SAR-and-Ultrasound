...
for R = 1+floor(window_size/2):image_row_size-floor(window_size/2)      % for every row 
    for C = 1+floor(window_size/2):image_col_size-floor(window_size/2)  % for every column
        ...
        % Call appropriate filter function to obtain the padded filtered matrix
        % run the mean filter if user entered mean
        if filter == "mean"                     
        ...
        elseif filter == "efficient median"
            [new_image, first_col, previous_hist] = image_efficient_median_filter(R, C, new_image, pixels_in_window, window_size, first_col, previous_hist); 
        ...
        elseif filter == "trimmed mean"
            new_image = image_trimmed_mean_filter(R, C, new_image, pixels_in_window, window_size);
        end
    end
end
